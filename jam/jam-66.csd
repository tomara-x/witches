...
instr Soil
tree_init(8, 8, 16)                 ;8 nodes, 8 values and 16 branches each
iarr[] = fillarray(1, 2)
...
;every time node 0 is active (only on the trigger)
if kN == 0 && kD[kS] == 1 then
    ;set value 0 of node 2 to a random note from scale
    node_set_value_k(2, 0, table:k(random:k(0,32), giScale))
endif
...
aSig *= db(linsegr(-12,4,-128))
sbus_mix 0, aSig
endin
schedule("Water", 0, 55) ;run for 55 seconds (+4 of fade out)
